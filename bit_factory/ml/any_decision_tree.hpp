...
struct decision_tree {
  ...
  struct print_node {
    ...
    friend std::ostream& operator<<(std::ostream& os, print_node const& p) {
      if (auto result = std::get_if<result_counts_t>(&p.node.node_data)) {
        ...
        os << print_coumn_value{p.sheet_, p.node.column_value};
        if (children.true_path)
          os << p.indent << "T-> "
             << print_node{p.sheet_, *children.true_path, p.indent + "   "};
        if (children.false_path)
          os << p.indent << "F-> "
             << print_node{p.sheet_, *children.false_path, p.indent + "   "};
        return os;
      }
    }
  };

  [[nodiscard]] std::string to_string(tree_t const& tree) {
    std::stringstream s;
    s << decision_tree::print_node{sheet_, tree, ""};
    return s.str();
  }
  ...
  value<> get_predict_value(row<> const& r) {
    return r[sheet_.column_count() - 1];
  }

  [[nodiscard]] result_counts_t result_counts(auto const& get_rows) {
    result_counts_t counts;
    for (auto const& row : get_rows()) ++counts[get_predict_value(row)];
    return counts;
  }
  ...
  [[nodiscard]] tree_t build_tree(auto const& get_rows, auto score_function) {
    if (auto best_gain = find_best_gain(
            get_rows, gain_t{.gain = 0.0, .criteria = {}, .split_sets = {}},
            score_function(result_counts(get_rows)), score_function);
        best_gain.gain > 0) {
      ...
      return tree_t{.column_value = {}, .node_data = result_counts(get_rows)};
