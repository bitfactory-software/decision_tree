...
//       nn::query_t{
//           db, db.get_io_ids({"Bank", "World"}, {"WorldBank", "River",
//           "Earth"})} .feed_forward();
...
void predict(nn::data_base_t& db, auto observation, int length = 9) {
  ...
  std::println("prediction for {} is {}", std::string_view{observation},
               std::views::zip(prediction_for_observation, io_ids.out));
}
...
void train_db(nn::data_base_t& db, auto const& data, bool log = false,
  ...
  for (auto line : std::views::split(data, nl)) {
    ...
    trace_nn(db, std::string_view(line));
