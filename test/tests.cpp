...
using behavior_to_decision_table_t =
    rows_t<std::string, std::string, bool, int, std::string>;
behavior_to_decision_table_t rows = {
...
TEST_CASE("divide_table_column") {
  auto split = divide_table_column<2>(rows, true);

  CHECK(split[0].size() == 8);
  std::println("split[0] {}", split[0]);
  behavior_to_decision_table_t expected_0 = {
      {"Slashdot", "USA", true, 18, "None"},
      {"Google", "France", true, 23, "Premium"},
      {"Digg", "USA", true, 24, "Basic"},
      {"Kiwitobes", "France", true, 23, "Basic"},
  ...
  behavior_to_decision_table_t expected_1 = {
      {"Google", "UK", false, 21, "Premium"},
      {"(direct)", "UK", false, 21, "Basic"},
      {"Google", "USA", false, 24, "Premium"},
      {"Digg", "USA", false, 18, "None"},
